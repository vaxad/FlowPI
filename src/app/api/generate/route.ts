...
export async function POST(req: NextRequest) {
  ...
  const rootDir = path.resolve(process.cwd()); // Directory where backend will be generated
...
const zipFilePath = path.join(publicDir, `${name}.zip`);  // Path for the zip file
...
`;
  ...
  try {
    ...
    return NextResponse.json({ message: 'Backend API generated!' });
  } catch (error) {
